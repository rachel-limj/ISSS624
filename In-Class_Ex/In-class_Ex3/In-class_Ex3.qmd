...
5.3.2 Installing and loading R packages
...
### 5.5.1.1 EDA using statistical graphics
...
The histogram and box plot of the newly derived variables (i.e. Radio penetration rate) is more evenly distributed, versus the original variable (i.e. Radio) which is more skewed to the left.

### 5.5.1.3 Plotting multiple histogram

In the figure below, multiple histograms are plotted to reveal the distribution of the selected variables in the *ict_derived* data.frame. The code chunks below are used to create the data visualisation. They consist of two main parts. First, we will create the individual histograms using the code chunk below.
...
When we plot Radio penetration rate against the total number of households, the chloropeth maps are no longer similar. For example, at top left hand area of map, the total number of households is relatively low but the radio penetration rate is the highest.
...
### t5.7.8 Selecting the optimal clustering algorithm
...
-   Alpha: Incremental units (0.1 -1)

-   K: Upper case

-   If Alpha used is 0.3:

    -   Attribute of Homogeneity loss / decrease is 20%.

    -   Spatial Homogeneity increase by 75%

-   Check for compromise points.
