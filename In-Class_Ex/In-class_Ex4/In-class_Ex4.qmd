---
title: "In-class_Ex4 - Calibrating Hedonic Pricing Model for Private Highrise Property with GWR Method"
editor: visual
---
...
### 6.1.1 Getting Started - R Packages
...
-   **GWmodel** - R package for calibrating geographical weighted family of models. This package provides a collection of **localised spatial statistical methods**, namely:

    -   GW summary statistics

    -   GW principal components analysis,

    -   GW discriminant analysis and various forms of GW regression; some of which are provided in basic and robust (outlier resistant) forms.

    -   Can build predictive model as well not just descriptive models
...
### 6.2.2 Updating CRS information
...
### 6.2.3 Check for Duplicate Data

```{r}
glimpse(mpsz_svy21)
```

Check for duplicated Subzone Code

```{r}
mpsz_svy21 <- (mpsz_svy21[order(mpsz_svy21$SUBZONE_C), ])

duplicate_area <- mpsz_svy21$SUBZONE_C[ mpsz_svy21$SUBZONE_C %in% mpsz_svy21$SUBZONE_C[duplicated(mpsz_svy21$SUBZONE_C)] ]

duplicate_area
```

No duplicated subzones

## 6.3 Aspatial Data Wrangling

### 6.3.1 Importing the aspatial data
...
### 6.3.2 Converting aspatial data frame into a sf object
...
### 6.4.1 EDA using statistical graphics
...
### 6.4.2 Multiple Histogram Plots distribution of variables
...
## 6.5 Drawing Statistical Point Map
...
## 6.6 Hedonic Pricing Modelling in R
...
### 6.6.1 Simple Linear Regression Method
...
### 6.6.2 Multiple Linear Regression Method

#### 6.6.2.1 Visualising the relationships of the independent variables
...
### 6.6.3 Building a hedonic pricing model using multiple linear regression method
...
### 6.6.4 Preparing Publication Quality Table: gtsummary method
...
6.6.5 Preparing Publication Quality Table: gtsummary method
...
6.6.5.1 Checking for multicolinearity
...
#### 6.6.5.2 Test for Non-Linearity
...
#### 6.6.3.3 Test for Normality Assumption
...
#### 6.6.5.4 Testing for Spatial Autocorrelation
...
## 6.7 Building Hedonic Pricing Models using GWmodel
...
### 6.7.1 Building Fixed Bandwidth GWR Model

#### 6.7.1.1 Computing fixed bandwith
...
Our projected system is in meters. Hence, the results are in meters.

#### 6.7.1.2 GWModel method - fixed bandwith
...
The report (GW - Geo weighted model) shows that the AICc of the gwr is 42263.61 which is significantly smaller than the global (non-geographical) multiple linear regression model of 42967.1. The former GW model perform better than global (non-GW) based on AICc

### 6.7.2 Building Adaptive Bandwidth GWR Model

In this section, we will calibrate the gwr-absed hedonic pricing model by using adaptive bandwidth approach.

#### 6.7.2.1 Computing the adaptive bandwidth
...
#### 6.7.2.2 Constructing the adaptive bandwidth gwr model
...
By default, you will need to input longlat into algorithm.

-   longlat = TRUE (Input data not projected, in decimal place)

-   longlat = FALSE (Input data has been projected)
...
### 6.7.4 Converting SDF into **sf** data.frame
...
tm_shape(condo_resale.sf.adaptive) +  
  tm_dots(col = "Local_R2",
          border.col = "gray60",
          border.lwd = 1) +
  tm_view(set.zoom.limits = c(11,14)) #set zoom limit
```
...
#### 6.7.6 Visualizing coefficient estimates
...
#### 6.7.6.1 By URA Planning Region
