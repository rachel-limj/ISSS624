...
In this in-class exercise, I learnt how to compute spatial weights using R:
...
To ensure **spdep**, **sf**, **tmap** and **tidyverse** packages of R are currently installed:

```{r}
pacman::p_load(sf, spdep, tmap, tidyverse)
```

### 3.1.2 Getting the Data Into R Environment

Import a geospatial data and its associated attribute table into R environment.

The geospatial data is in ESRI shapefile format and the attribute table is in csv fomat.

### 3.1.3 Import shapefile into R environment
...
### 3.1.4 Import csv file into R environment

Next, import *Hunan_2012.csv* into R by using *read_csv()* of **readr** package.
...
### 3.1.5 Performing relational join
...
## 3.2 Visualizing Regional Development Indicator
...
## 
3.3 Computing Contiguity Spatial Weights

In this section, I learnt how to use [*poly2nb()*](https://r-spatial.github.io/spdep/reference/poly2nb.html) of **spdep** package to **compute contiguity weight matrices** for the study area. This function builds a neighbours list based on regions with contiguous boundaries.

Passing a \"queen\" argument that takes TRUE or FALSE as options. The argument by default is set to TRUE, and returns a list of first order neighbours using the Queen criteria.

### 3.3.1 Computing (QUEEN) contiguity based neighbours
...
The summary report above shows:
...
For each polygon in polygon object, *wm_q* lists all neighboring polygons. For example, to see the neighbors for the first polygon in the object, type:
...
GDPPC stands for GDP per capita
...
### 3.3.2 Creating (ROOK) contiguity based neighbours
...
### 3.3.3 Visualizing contiguity weights
...
#### 3.3.3.1 Plotting Queen contiguity based neighbours map
...
#### 3.3.3.2 Plotting Rook contiguity based neighbours map
...
#### 3.3.3.3 Plotting both Queen and Rook contiguity based neighbours maps
...
In this section, I learnt how to derive **distance-based weight matrices** by using [dnearneigh()](https://r-spatial.github.io/spdep/reference/dnearneigh.html) of spdep package.
...
**Answer:** Average number of links refers to the average number of neighbouring points each point has based on the defined upper and lower limit boundaries. It's also the K.
...
#### 
3.4.2.1 Plotting fixed distance weight matrix
...
### 
3.4.3 Computing adaptive distance weight matrix
...
#### 
3.4.3.1 Plotting distance based neighbours
...
### 3.5.1 Row-standardised weights matrix
...
## 
3.6 Application of Spatial Weight Matrix
...
### 
3.6.1 Spatial lag with row-standardized weights

Finally, we\'ll compute the [average neighbor GDPPC value]{.underline} for each polygon. These values are often referred to as **spatially lagged values**.
...
```{r}
GDPPC.lag.b <- lag.listw(rswm_ids, hunan$GDPPC)
GDPPC.lag.b
```
...
**Answer:** The spatial lag value is computed using the sum of the equal weights \* values (GDPPC)

20981 \* 0.2 + 34592 \* 0.2 + 24473 \* 0.2 + 21311 \* 0.2 + 22879 \* 0.2 = 24847.20
...
### 
3.6.2 Spatial lag as a sum of neighboring values
...
**Answer:** Yes its the summation of neighbouring GDPPC Values of each county
